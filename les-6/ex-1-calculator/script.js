...
const productResultContainer = document.querySelector(
...
function removeNumberInput(eventInformation) {
    ...
    console.log(
        eventInformation.target,
        eventInformation.target.matches('.delete-number-input')
    );

    if (
        eventInformation.target.matches('.delete-number-input') &&
        numberInputContainer.children.length > 2
    ) {
        const selectedInputGroup = eventInformation.target.closest(
            '.number-input-group'
        );
